...
export async function handleTabCreated(tab: chrome.tabs.Tab): Promise<void> {
  ...
  if (config?.distractionBlocker.enabled) {
    ...
    const isDistracting = distractingDomains.some(
      (d: DistractingDomain) => d.domain === domain
    );
  ...
  try {
    ...
    if (config?.tabLimiter.enabled) {
      ...
      if (validTabs.length > maxTabs) {
        // Find tabs to close (keep the most recently used ones)
        // Since chrome.tabs.Tab does not have lastAccessed, fallback to tab.id as a proxy for recency (not perfect, but deterministic)
        // Optionally, you could store lastAccessed in extension storage for more accuracy
...
export async function handleTabUpdated(
  ...
  if (changeInfo.status === 'complete' && tab.url) {
    ...
    try {
      ...
      if (config?.distractionBlocker?.enabled) {
        const domain = extractDomain(tab.url);
        const distractingDomain = config.distractionBlocker.domains.find(
          (d: DistractingDomain) => d.domain === domain
        );
