...
const BlockerSettings: React.FC<BlockerSettingsProps> = ({ onNavigateBack }) => {
  ...
  const addDomain = async () => {
    ...
    if (domainName) {
      ...
      const updatedDomains = [newDomainObj, ...config.distractionBlocker.domains];
      
      try {
        await updateConfig('distractionBlocker.domains', updatedDomains);
        setNewDomain('');
            ...
            <div key={index} className="bg-bg-secondary rounded-apple p-md border border-bg-tertiary">
              <div className="flex items-center justify-between">
                <div className="flex-1">
                  ...
                  <div className="text-xs text-text-secondary">Shared daily limit applies</div>
                </div>
                <button
      ...
      <div className="flex flex-col gap-md">
          ...
          <div className="flex flex-col gap-sm">
            ...
            <div className="text-xs text-text-secondary">
              This domain will be blocked immediately
            </div>
          </div>
        </div>
      </div>
